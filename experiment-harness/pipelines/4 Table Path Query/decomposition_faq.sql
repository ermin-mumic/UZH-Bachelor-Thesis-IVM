...
SELECT CUSTKEY, COUNT(*) AS COUNT_C1
FROM CHILD_1_Q
GROUP BY CUSTKEY;
...
SELECT ORDERKEY, SUM(COUNT_C1) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY ORDERKEY;
...
SELECT PARTKEY, SUPPKEY, COUNT(*) AS COUNT_C3
FROM CHILD_3_Q
...
SELECT SUM(COUNT_C2 * COUNT_C3) AS total_count
FROM ROOT_Q;
