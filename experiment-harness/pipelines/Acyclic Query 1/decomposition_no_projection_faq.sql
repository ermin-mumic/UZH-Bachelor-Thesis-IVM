...
SELECT info_type_id, COUNT(*) AS COUNT_C1
FROM CHILD_1_Q
...
SELECT movie_id, COUNT(*) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY movie_id;
...
SELECT company_type_id, COUNT(*) AS COUNT_C3
FROM CHILD_3_Q
...
SELECT *
FROM movie_info_idx
JOIN CHILD_1_P USING (info_type_id);

CREATE MATERIALIZED VIEW CHILD_4_P AS
SELECT movie_id, SUM(COUNT_C1) AS COUNT_C4
FROM CHILD_4_Q
GROUP BY movie_id;
...
JOIN CHILD_2_P USING (movie_id)
JOIN CHILD_3_P USING (company_type_id);

-- Result.
CREATE MATERIALIZED VIEW RESULT AS
SELECT SUM(COUNT_C4 * COUNT_C2 * COUNT_C3) AS total_count
FROM ROOT_Q;
