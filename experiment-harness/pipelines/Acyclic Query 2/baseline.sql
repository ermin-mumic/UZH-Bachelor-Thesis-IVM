...
SELECT *
FROM company_name
JOIN movie_companies USING (company_id)
JOIN title USING (movie_id)
JOIN movie_info USING (movie_id)
JOIN info_type USING (info_type_id)
JOIN cast_info USING (movie_id)
JOIN char_name USING (char_id)
JOIN role_type USING (role_id)
JOIN aka_name USING (person_id)
JOIN name USING (person_id);
