...
SELECT A, D, SUM(COUNT_C1) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY A, D;

-- Child Bag 3.
CREATE MATERIALIZED VIEW CHILD_3_Q AS
SELECT R7.A AS A, R8.D AS D, R7.E AS E, COUNT_C2
FROM R4
...
SELECT A, E, SUM(COUNT_C2) AS COUNT_C3
FROM CHILD_3_Q
GROUP BY A, E;

-- Root Bag.
CREATE MATERIALIZED VIEW ROOT_Q AS
SELECT R7.A AS A, R5.E AS E, R6.F AS F, COUNT_C3
FROM R5
