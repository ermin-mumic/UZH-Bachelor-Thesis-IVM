...
SELECT CUSTKEY, NATIONKEY, COUNT(*) AS COUNT_C1
FROM CHILD_1_Q
...
SELECT SUPPKEY, NATIONKEY, COUNT(*) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY SUPPKEY, NATIONKEY;
...
SELECT ORDERKEY, CUSTKEY, COUNT(*) AS COUNT_C3
FROM CHILD_3_Q
...
USING (SUPPKEY)
JOIN CHILD_1_P USING (CUSTKEY, NATIONKEY)
JOIN CHILD_2_P USING (SUPPKEY, NATIONKEY);

CREATE MATERIALIZED VIEW CHILD_4_P AS
SELECT SUPPKEY, CUSTKEY, SUM(COUNT_C1 * COUNT_C2) AS COUNT_C4
FROM CHILD_4_Q
GROUP BY SUPPKEY, CUSTKEY;
...
USING (CUSTKEY)
JOIN CHILD_3_P USING (ORDERKEY, CUSTKEY)
JOIN CHILD_4_P USING (SUPPKEY, CUSTKEY);

CREATE MATERIALIZED VIEW CHILD_5_P AS
SELECT ORDERKEY, SUPPKEY, SUM(COUNT_C4 * COUNT_C3) AS COUNT_C5
FROM CHILD_5_Q
...
SELECT SUM(COUNT_C5) AS total_count
FROM ROOT_Q;
