...
SELECT person_id, COUNT(*) AS COUNT_C1
FROM CHILD_1_Q
GROUP BY person_id;
...
SELECT role_id, COUNT(*) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY role_id;
...
SELECT person_id, COUNT(*) AS COUNT_C3
FROM CHILD_3_Q
GROUP BY person_id;
...
SELECT company_id, COUNT(*) AS COUNT_C4
FROM CHILD_4_Q
GROUP BY company_id;
...
JOIN CHILD_4_P USING (company_id);

CREATE MATERIALIZED VIEW CHILD_5_P AS
SELECT movie_id, SUM(COUNT_C4) AS COUNT_C5
FROM CHILD_5_Q
GROUP BY movie_id;
...
SELECT char_id, COUNT(*) AS COUNT_C6
FROM CHILD_6_Q
GROUP BY char_id;
...
SELECT movie_id, COUNT(*) AS COUNT_C7
FROM CHILD_7_Q
GROUP BY movie_id;
...
SELECT *
FROM cast_info
JOIN CHILD_1_P USING (person_id)
JOIN CHILD_2_P USING (role_id)
JOIN CHILD_3_P USING (person_id)
JOIN CHILD_5_P USING (movie_id)
JOIN CHILD_6_P USING (char_id)
JOIN CHILD_7_P USING (movie_id);

CREATE MATERIALIZED VIEW CHILD_8_P AS
SELECT movie_id, SUM(COUNT_C1 * COUNT_C2 * COUNT_C3 * COUNT_C5 * COUNT_C6 * COUNT_C7) AS COUNT_C8
FROM CHILD_8_Q
GROUP BY movie_id;
...
SELECT *
FROM movie_info
JOIN CHILD_8_P USING (movie_id);

CREATE MATERIALIZED VIEW CHILD_9_P AS
SELECT info_type_id, SUM(COUNT_C8) AS COUNT_C9
FROM CHILD_9_Q
...
SELECT *
FROM info_type
JOIN CHILD_9_P USING (info_type_id);

-- Result.
CREATE MATERIALIZED VIEW RESULT AS
SELECT SUM(COUNT_C9) AS total_count
FROM ROOT_Q;