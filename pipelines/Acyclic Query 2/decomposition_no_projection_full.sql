...
SELECT *
FROM ROOT_Q
JOIN CHILD_5_Q USING (person_id)
JOIN CHILD_6_Q USING (role_id)
JOIN CHILD_7_Q USING (person_id)
JOIN CHILD_3_Q USING (movie_id)
JOIN CHILD_1_Q USING (company_id)
JOIN CHILD_8_Q USING (char_id)
JOIN CHILD_4_Q USING (movie_id)
JOIN CHILD_2_Q USING (info_type_id)
JOIN CHILD_9_Q USING (movie_id);