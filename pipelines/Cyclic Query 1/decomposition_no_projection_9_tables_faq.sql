...
SELECT A, E, COUNT(*) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY A, E;

-- Child Bag 3.
CREATE MATERIALIZED VIEW CHILD_3_Q AS
SELECT R1.A AS A, R9.B AS B, R8.D AS D, COUNT_C1
FROM R1
...
SELECT A, D, SUM(COUNT_C1) AS COUNT_C3
FROM CHILD_3_Q
GROUP BY A, D;

-- Root Bag.
CREATE MATERIALIZED VIEW ROOT_Q AS
SELECT R7.A AS A, R8.D AS D, R7.E AS E, COUNT_C3, COUNT_C2
FROM R4
...
SELECT SUM(COUNT_C3 * COUNT_C2) AS total_count
FROM ROOT_Q;
