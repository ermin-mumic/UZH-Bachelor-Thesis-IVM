...
SELECT R1.A AS A, R1.B AS B, R7.E AS E, COUNT_C1
FROM R1
...
SELECT B, E, SUM(COUNT_C1) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY B, E;
...
SELECT B, D, SUM(COUNT_C2) AS COUNT_C3
FROM CHILD_3_Q
GROUP BY B, D;

-- Root Bag.
CREATE MATERIALIZED VIEW ROOT_Q AS
SELECT R2.B AS B, R3.C AS C, R8.D AS D, COUNT_C3
FROM R2
