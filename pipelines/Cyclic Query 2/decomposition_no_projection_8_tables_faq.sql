...
SELECT B, D, COUNT(*) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY B, D;

-- Child Bag 3.
CREATE MATERIALIZED VIEW CHILD_3_Q AS
SELECT R8.B AS B, R4.D AS D, R4.E AS E, COUNT_C2
FROM R4
...
SELECT B, E, SUM(COUNT_C2) AS COUNT_C3
FROM CHILD_3_Q
GROUP BY B, E;

-- Root Bag.
CREATE MATERIALIZED VIEW ROOT_Q AS
SELECT R1.A AS A, R1.B AS B, R7.E AS E, COUNT_C3, COUNT_C1
FROM R1
...
SELECT SUM(COUNT_C3 * COUNT_C1) AS total_count
FROM ROOT_Q;
