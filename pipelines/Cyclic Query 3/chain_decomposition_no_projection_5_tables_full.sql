-- Template.
CREATE TABLE R1 (
    A bigint,
    B bigint
) WITH (
    'materialized' = 'true',
    'connectors' = '[{
        "name": "R1_connector",
        "transport": {
            "name": "file_input",
            "config": {
                "path": "/{{FILE_PATH}}"
            }
        },
        "format": { 
            "name": "csv",
            "config": {
                "headers": true
            }
        }
    }]'
);

CREATE TABLE R2 (
    B bigint,
    C bigint
) WITH (
    'materialized' = 'true',
    'connectors' = '[{
        "name": "R2_connector",
        "transport": {
            "name": "file_input",
            "config": {
                "path": "/{{FILE_PATH}}"
            }
        },
        "format": { 
            "name": "csv",
            "config": {
                "headers": true
            }
        }
    }]'
);

CREATE TABLE R3 (
    C bigint,
    D bigint
) WITH (
    'materialized' = 'true',
    'connectors' = '[{
        "name": "R3_connector",
        "transport": {
            "name": "file_input",
            "config": {
                "path": "/{{FILE_PATH}}"
            }
        },
        "format": { 
            "name": "csv",
            "config": {
                "headers": true
            }
        }
    }]'
);

CREATE TABLE R4 (
    B bigint,
    D bigint
) WITH (
    'materialized' = 'true',
    'connectors' = '[{
        "name": "R4_connector",
        "transport": {
            "name": "file_input",
            "config": {
                "path": "/{{FILE_PATH}}"
            }
        },
        "format": { 
            "name": "csv",
            "config": {
                "headers": true
            }
        }
    }]'
);

CREATE TABLE R5 (
    A bigint,
    D bigint
) WITH (
    'materialized' = 'true',
    'connectors' = '[{
        "name": "R5_connector",
        "transport": {
            "name": "file_input",
            "config": {
                "path": "/{{FILE_PATH}}"
            }
        },
        "format": { 
            "name": "csv",
            "config": {
                "headers": true
            }
        }
    }]'
);

-- Child Bag 1.
CREATE MATERIALIZED VIEW CHILD_1_Q AS
SELECT R2.B AS B, R3.C AS C, R4.D AS D
FROM R2
JOIN R3 ON R2.C = R3.C
JOIN R4 ON R4.B = R2.B AND R4.D = R3.D;

CREATE MATERIALIZED VIEW CHILD_1_P AS
SELECT DISTINCT B, D
FROM CHILD_1_Q;

-- Root Bag.
CREATE MATERIALIZED VIEW ROOT_Q AS
SELECT R1.A AS A, R4.B AS B, R5.D AS D
FROM R1
JOIN R4 ON R4.B = R1.B
JOIN R5 ON R5.A = R1.A AND R5.D = R4.D
JOIN CHILD_1_P ON CHILD_1_P.B = R4.B AND (CHILD_1_P.B = R1.B) AND CHILD_1_P.D = R5.D AND (CHILD_1_P.D = R4.D);

-- Result.
CREATE MATERIALIZED VIEW RESULT AS
SELECT ROOT_Q.A AS A, ROOT_Q.B AS B, CHILD_1_Q.C AS C, CHILD_1_Q.D AS D
FROM ROOT_Q
JOIN CHILD_1_Q ON CHILD_1_Q.B = ROOT_Q.B AND CHILD_1_Q.D = ROOT_Q.D;