...
CREATE TABLE person_info (
    ...
    'connectors' = '[{
        "transport": {
            "name": "file_input",
            "config": {
                "path": "/{{FILE_PATH}}/person_info.csv"
            }
        },
        "format": { 
            "name": "csv",
...
SELECT movie_id, person_id, COUNT(*) AS COUNT_C1
FROM CHILD_1_Q
...
SELECT info_type_id, person_id, COUNT(*) AS COUNT_C2
FROM CHILD_2_Q
GROUP BY info_type_id, person_id;
...
SELECT movie_id, info_type_id, COUNT(*) AS COUNT_C3
FROM CHILD_3_Q
...
SELECT *
FROM CHILD_1_P
JOIN CHILD_2_P USING (person_id)
JOIN CHILD_3_P USING (movie_id, info_type_id)
JOIN (
    SELECT DISTINCT person_id, movie_id
    FROM cast_info
)
USING (person_id, movie_id)
JOIN (
    SELECT DISTINCT info_type_id, person_id
    FROM person_info
)
USING (info_type_id, person_id)
JOIN (
    SELECT DISTINCT movie_id, info_type_id
    FROM movie_info
)
USING (movie_id, info_type_id);

-- Result.
CREATE MATERIALIZED VIEW RESULT AS
SELECT SUM(COUNT_C1 * COUNT_C2 * COUNT_C3) AS total_count
FROM ROOT_Q;
