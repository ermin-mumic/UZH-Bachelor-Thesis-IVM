...
We fix our data model and query class and introduce a running example we revisit throughout, state the insert-only maintenance problem the approach addresses, and present tree decompositions and the fractional hypertree width derived from them.
...
The \emph{size} $|R|$ of a relation is the number of its tuples, and the size $|\mathcal{D}|$ of a database is the sum of the sizes of its relations.
...
\] 
where $R_i$ is a relation symbol and $\mathbf{X}_i$ is a tuple of variables, the \emph{schema} of $R_i$. Each $R_i(\mathbf{X}_i)$ is an \emph{atom} of $Q$, and $\mathrm{vars}(Q) = \bigcup_{i=1}^{k} \mathbf{X}_i$ denotes the set of all variables of $Q$.
...
As a running example we use a small query over a single binary relation $E$, which we read as the edge set of a directed graph.
Consider
\[
...
\section{The Insert-Only Maintenance Problem}
...
The database $\mathcal{D}$ is initially empty and grows by single-tuple insertions, applied one at a time, and neither their number nor which tuples they insert is known in advance.
Writing $\mathcal{D}^{(0)} = \emptyset$ and $\mathcal{D}^{(\tau)}$ for the database after the $\tau$-th insertion, the task is to maintain the result $Q(\mathcal{D}^{(\tau)})$ at each time-step $\tau$.
...
Let $\mathbf{V} \subseteq \mathrm{vars}(Q)$ be a set of variables.
A \emph{fractional edge cover}~\cite{atserias2013} of $\mathbf{V}$ assigns a weight $\lambda_i \in [0,1]$ to each atom $R_i(\mathbf{X}_i)$ of $Q$ such that, for every variable $x \in \mathbf{V}$, the weights of the atoms whose schema contains $x$ sum to at least one.
...
\]
In particular, a tree decomposition with a single bag has that bag equal to $\mathrm{vars}(Q)$, so the maximum over its bags is just $\rho^\ast(\mathrm{vars}(Q))$; since $w(Q)$ minimises over all tree decompositions, $w(Q) \le \rho^\ast(\mathrm{vars}(Q))$.
...
Concretely, when each bag is evaluated with a worst-case-optimal join, a sequence of $N$ insertions is maintained in total time $O(N^{w(Q)})$. 
Acyclic queries have $w(Q) = 1$, since every atom can occupy its own bag, whereas a cycle forces some bag to cover more than one atom, giving $w(Q) > 1$.
