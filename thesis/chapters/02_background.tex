...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[bag/.style={draw, ellipse, minimum width=24mm, minimum height=11mm, inner sep=2pt}]
    \node[bag] (r) at (0,1.9) {$\{A,B,D\}$};
    \node[bag] (c) at (0,0)   {$\{B,C,D\}$};
    \draw (r) -- (c);
  \end{tikzpicture}
...
What $w(Q)$ means for the cost of maintaining a query is taken up in Chapter~\ref{ch:ivm}.
...
The IVM$^+$ approach is built on a tree decomposition of minimum fractional hypertree width.
Computing such a decomposition is NP-hard~\cite{fischl2018}, and approximation algorithms exist~\cite{marx2010}.
Our decomposer instead uses a practical heuristic: it generates candidate decompositions with a min-fill heuristic and selects the one of least fractional hypertree width, as described in more detail in Chapter~\ref{ch:implementation}.
