...
Feldera is a streaming query engine for \emph{incremental computation}~\cite{feldera2026}.
...
We chose them over HTTP ingress because posting the data over HTTP could not keep the pipeline fed: even when sent in batches, each batch was processed almost instantly and the pipeline then idled until the next arrived, so a run measured how fast data could be delivered rather than how fast Feldera maintained the views. Reading directly from local CSV files through the connectors keeps the engine saturated, so that the incremental computation is what is measured.
...
A table or view can be declared \emph{materialised} to keep its full contents available, for example to inspect or query them directly.
A table with a \texttt{PRIMARY KEY} is materialised automatically, since Feldera must maintain a key index.
Materialising a table or view can slow the pipeline down, as more data must be written to storage, which can also increase the pipeline's memory use.
...
The view is materialised and can therefore be queried directly (Section~\ref{sec:feldera}).
...
The same semiring construction handles other aggregates. One that reads a column, such as \texttt{MIN}, carries its value explicitly and combines it in a single \emph{canonical} bag so that it is not aggregated more than once. We implement \texttt{COUNT(*)} as a representative case.
...
It could also be handled directly by counting each atom in a single \emph{canonical} bag and joining a deduplicated projection of it elsewhere, the same mechanism used for \texttt{MIN} above.
...
Parsing itself uses \emph{sqlglot}, a Python SQL parsing library, which turns the SQL into an abstract syntax tree that we then traverse.
...
It relies on the Minimum Fill-in heuristic~\cite{bodlaender2005} from the \emph{networkx} library, which operates on ordinary graphs rather than hypergraphs, so the stage first builds the \emph{primal graph} of the hypergraph: each hyperedge, that is each relation's set of variables, becomes a clique.
...
\begin{sidewaysfigure}[p]
  \centering
  \includegraphics[width=\textheight]{./figures/ch4_web_ui.png}
...
None of these limitations is fundamental: they reflect the tool's scope as a minimal prototype, and nothing in the approach prevents a future version from supporting the full query surface, further aggregates, and richer joins and filters.
